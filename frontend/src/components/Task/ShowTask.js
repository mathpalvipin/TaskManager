...
const ShowTask = ({ currentDate, setCurrentDate }) => {
  ...
  useEffect(() => {
       ...
       const selectedDiv= showContainerRef?.current?.querySelectorAll('[data-name="highlighted"]');
        ...
        Tasks.map((task) => {
          ...
          return (
            <div
              ...
              key={task._id}
            >
            <div className="flex-col">  <div className={classes.TaskName}>{task.TaskName}</div>
              <div className={classes.TaskType}>{task.TaskType}</div>
              </div>
             <div className="flex-col"> <div className={classes.DateTime}>{task.DateTime}</div>
              <button
